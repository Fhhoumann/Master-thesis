...
MAIN_SPECS = [
    RunSpec(
        key="EN_twostep",
        pipeline="twostep",
        cfg="final_twostep_elasticnet_rolling_nocost.yaml",
        artifact_run_name="final_twostep_elasticnet_rolling_nocost_v1",
    ),
    RunSpec(
        key="RF_twostep",
        pipeline="twostep",
        cfg="final_twostep_random_forest_rolling_nocost.yaml",
        artifact_run_name="final_twostep_random_forest_rolling_nocost_v2",
    ),
    RunSpec(
        key="Torch_onestep",
        pipeline="onestep",
        cfg="final_onestep_torch_rolling_nocost_v2.yaml",
        artifact_run_name="final_onestep_torch_rolling_nocost_v2",
    ),
]
...
def _load_nocost_backtest(spec: RunSpec) -> pd.DataFrame:
    ...
    # Older/newer backtest artifacts may not carry both turnover columns.
    if "turnover_one_way" not in bt.columns:
        bt["turnover_one_way"] = np.nan
    if "turnover_round_trip" not in bt.columns:
        bt["turnover_round_trip"] = np.nan
    bt["run_key"] = spec.key
    return bt
...
def _load_ff_with_umd_monthly() -> pd.DataFrame:
    ff_cache = ARTIFACTS / "factors" / "ken_french_monthly.parquet"
    umd_cache = ARTIFACTS / "factors" / "ken_french_momentum_monthly.parquet"
    ff = load_factor_panel(ff_cache) if ff_cache.exists() else fetch_ken_french_monthly(ff_cache, refresh=False)
    umd = (
        load_umd_factor_panel(umd_cache)
        if umd_cache.exists()
        else fetch_ken_french_momentum_monthly(umd_cache, refresh=False)
    )
    out = ff.merge(umd, on="date", how="inner")
...
def build_main_outputs() -> None:
    ...
    model_start = merged_bt["date"].min()
    model_end = merged_bt["date"].max()
    bench = bench[(bench["date"] >= model_start) & (bench["date"] <= model_end)].copy()
...
def build_ff3_carhart_outputs() -> None:
    backtests = [_load_nocost_backtest(spec) for spec in MAIN_SPECS]
    merged_bt = pd.concat(backtests, ignore_index=True)
    ...
    factors = _load_ff_with_umd_monthly()
    factors["date"] = pd.to_datetime(factors["date"], errors="coerce")
    factors = factors[["date", "rf", "mkt_rf", "smb", "hml", "umd"]].dropna()
    ...
    out = out[(out["regime"] == "full_sample") & (out["model"].isin(["ff3", "carhart4"]))].copy()
    out = out[
        [
            "run_key",
            "model",
    ...
    ].sort_values(["run_key", "model"])
    out.to_csv(REPORT_TABLES / "final_120_ff3_carhart_table.csv", index=False)
...
def write_run_manifest(run_robustness: bool) -> None:
    manifest = {
        ...
        "outputs": {
            ...
            "ff3_carhart_table": str(REPORT_TABLES / "final_120_ff3_carhart_table.csv"),
